...
const allRoles = ["student", "teacher", "school_admin", "superadmin"] as const;
const roleLabel: Record<(typeof allRoles)[number], string> = { student: "Schüler:in", teacher: "Lehrkraft", school_admin: "Schul-Admin", superadmin: "Superadmin" };

export function UsersPage() {
  const ownRole = useAuthStore((s) => s.user?.role);
  const isSchoolAdmin = ownRole === "school_admin";
  const roles = (isSchoolAdmin ? ["teacher"] : [...allRoles]) as readonly (typeof allRoles)[number][];
  ...
  return <div className="p-8">
    <div className="mb-8 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-2xl font-semibold text-gray-900">Benutzerverwaltung</h1><p className="mt-2 text-sm text-gray-500">{isSchoolAdmin ? "Lehrkräfte deiner Schule anlegen und verwalten." : "Konten anlegen, Rollen verwalten und mehrere Nutzer:innen auf einmal importieren."}</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => setMode("bulk")}>Massenanlage</Button><Button onClick={() => setMode("single")}>Benutzer anlegen</Button></div></div>
    {mode === "single" && <UserCreateForm roles={roles} pending={create.isPending} error={create.isError} onCancel={() => setMode(null)} onSubmit={(data) => create.mutate(data)} />}
    {mode === "bulk" && <BulkCreateForm roles={roles} classes={classes.data ?? []} pending={bulk.isPending} error={bulk.isError} onCancel={() => setMode(null)} onSubmit={(data) => bulk.mutate(data)} />}
    {editing && <UserEditForm roles={roles} user={editing} pending={update.isPending} error={update.isError} onCancel={() => setEditing(undefined)} onSubmit={(data) => update.mutate({ id: editing.id, data })} />}
